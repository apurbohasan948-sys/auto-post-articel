...
export const ProviderModal: React.FC<ProviderModalProps> = ({
  ...
  // Authoritative real stored key from providerStore or provider prop (never the display mask)
  const storedProvider = provider?.id ? providerStore.getProviderById(provider.id) : null;
  const storedApiKey = (storedProvider?.apiKey || provider?.apiKey || '').trim();
  const hasStoredKey = Boolean(storedApiKey && !storedApiKey.includes('••••'));

  // Separate user input state so display mask is never used as actual state (Section 3)
  const [inputApiKey, setInputApiKey] = useState<string>('');
  const [isKeyCleared, setIsKeyCleared] = useState<boolean>(false);
  ...
  // Reset inputs when modal opens or provider changes
  useEffect(() => {
    setInputApiKey('');
  ...
  /**
   * Resolves the real API key according to Case A, B, C, D:
   * - CASE A: New provider -> use inputApiKey
   * - CASE B: Existing provider, no change -> preserve storedApiKey
   * - CASE C: Existing provider, new key typed -> use inputApiKey
   * - CASE D: Existing provider, explicitly cleared -> return ''
   */
  ...
  const handleRunModalTest = async () => {
    ...
    try {
      ...
      const payload: AIProviderConfig = {
        ...formData,
        apiKey: effectiveKey,
        hasKey: Boolean(effectiveKey && effectiveKey.length > 0),
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    let finalKey = resolveEffectiveApiKey();
    // Absolute safeguard: ensure mask '••••' is NEVER saved as key (Section 2)
    if (finalKey.includes('••••')) {
      finalKey = hasStoredKey ? storedApiKey : '';
    }

    onSave({
      ...formData,
      apiKey: finalKey,
      hasKey: Boolean(finalKey && finalKey.length > 0),
      modelName: formData.modelName || formData.defaultModel || 'gpt-4o',
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="text-[11px] font-mono">
                  {isKeyCleared ? (
                    <span className="text-rose-400">Key cleared (unsaved)</span>
                  ) : inputApiKey.trim() ? (
                    <span className="text-amber-400">New key entered</span>
                  ) : hasStoredKey ? (
                    <span className="text-emerald-400">Key configured</span>
                  ) : (
                    <span className="text-slate-500">No key stored</span>
                  )}
                </span>
                {hasStoredKey && !isKeyCleared && (
                  <button
                    type="button"
                    onClick={() => {
            ...
            <p className="text-[11px] text-slate-400">
              API keys are stored directly in your browser&apos;s localStorage (<code>tara_ai_api_providers</code>). Leaving this field blank preserves the existing key.
            </p>
          </div>
