...
export const ProvidersView: React.FC<ProvidersViewProps> = ({
  ...
  const handleSaveAiProvider = async (provider: AIProviderConfig) => {
    try {
      // Step 1: Save provider via apiClient (backed by providerStore/localStorage)
      ...
      // Step 2 & 3: Immediately read back from providerStore
      const verifiedList = providerStore.loadProviders();
      const savedProvider = verifiedList.find((p) => p.id === savedId);

      if (!savedProvider) {
        throw new Error('Provider could not be verified after save.');
      }

      // Step 4: Verify that if an unmasked key was entered, it persisted and is not masked
      if (provider.apiKey && !provider.apiKey.includes('••••')) {
        if (!savedProvider.apiKey || savedProvider.apiKey.includes('••••')) {
          throw new Error('API key verification failed: Key was not persisted or was masked.');
        }
      }

      // Step 5: Safe diagnostic log without logging actual keys
      console.log('[ProvidersView] Provider saved and verified:', {
        id: savedProvider.id,
        name: savedProvider.name,
        apiKeyPresent: Boolean(savedProvider.apiKey),
        apiKeyLength: savedProvider.apiKey ? savedProvider.apiKey.length : 0,
      });

      // Only then show confirmation
      showToast(`Provider "${savedProvider.name}" saved successfully.`);
  ...
  const handleTestAi = async (p: AIProviderConfig) => {
    ...
    try {
      // Section 7: Obtain fresh provider directly through providerStore (never stale React state)
      const freshStored = providerStore.getProviderById(p.id) || p;
      const res = await apiClient.testAIProvider({ providerId: p.id, provider: freshStored });
                          ...
                          <div>
                            ...
                            <span className="text-slate-300 flex items-center gap-1">
                              ...
                              {p.hasKey ? (
                                ...
                                <span className="text-rose-400 text-xs font-mono">Not configured</span>
                              )}
                            </span>
                          </div>
                        </div>
