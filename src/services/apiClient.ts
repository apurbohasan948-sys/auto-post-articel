...
export const apiClient = {
  ...
  saveAIProvider: async (
    ...
    // Section 6: LocalStorage Read-back Verification
    const verifiedList = providerStore.loadProviders();
    const verified = verifiedList.find((p) => p.id === saved!.id);
    if (!verified) {
      throw new Error(`Verification failed: Provider ${saved!.id} not found after write.`);
    }

    // If an unmasked key was provided, verify it persisted and is not masked
    if (provider.apiKey && !provider.apiKey.includes('••••')) {
      if (!verified.apiKey || verified.apiKey.includes('••••')) {
        throw new Error('Verification failed: Stored API key was lost or masked.');
      }
    }

    // Safe security log (Section 6: NEVER print actual key, log only presence and length)
    console.log('[ProviderStore] Verified provider in localStorage:', {
      id: verified.id,
      name: verified.name,
      apiKeyPresent: Boolean(verified.apiKey),
      apiKeyLength: verified.apiKey ? verified.apiKey.length : 0,
    });
  ...
  testAIProvider: async (params: {
    ...
    // Section 7: API Test MUST read the real saved key from providerStore directly
    const targetId = params.providerId || params.provider?.id;
    const stored = targetId ? providerStore.getProviderById(targetId) : null;

    let targetProvider: AIProviderConfig;
    if (stored) {
      // Use real stored key from providerStore
      let effectiveApiKey = stored.apiKey;
      // If modal passed an explicit new unmasked key for testing, use it
      if (params.provider?.apiKey && !params.provider.apiKey.includes('••••')) {
        effectiveApiKey = params.provider.apiKey.trim();
      }
      ...
      targetProvider = {
        ...
        hasKey: Boolean(effectiveApiKey && effectiveApiKey.length > 0),
      };
    } else if (params.provider) {
      targetProvider = params.provider;
    } else {
      throw new Error('No provider specified for API test');
    }
